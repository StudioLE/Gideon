...
module.exports = function() {
  ...
  async.waterfall([
    ...
    function(files, callback) { // INPUT: Select source directory
      ...
      terminal.write('Select a directory to backup', files, callback)
    ...
    function(db_found, callback) { // INPUT: Backup MySQL?
      ...
      if(db_found) {
        terminal.write('A database was found. Would you like to include it in the backup?', ['Yes', 'No'], callback)
      }
      else {
        callback(null, 'No', 1)
      }
    ...
    function(backup_database, index, callback) { // Backup directory & MySQL
      ...
      terminal.write(['Running backup', chalk.dim(output.name)])
      ...
      if(config.target == 'local') {
        ...
        output.stream.on('close', function() {
          callback(null, output.name, humanize.filesize(archive.pointer()))
        })
        ...
        */
        output.stream.on('part', function(details) {
          upload_size = humanize.filesize(details.uploadedSize)
          terminal.write(['Running backup', chalk.dim(output.name), 'Uploaded so far: ' + chalk.dim(upload_size)])
        })
        ...
        */
        output.stream.on('uploaded', function(details) {
          callback(null, output.name, upload_size)
        })
    ...
    if(err){
      if(err.message == 'Cancelled by user') {
        ...
        terminal.write(['Could not create archive', chalk.dim('Check directory exists')])
      }
      else if(_.isString(err) && err.substr(0, 41) === 'Failed to create a multipart upload on S3') {
        terminal.write(['Error occured connecting to S3', chalk.dim('Check your AWS credentials')])
      }
      else {
        terminal.end(err)
      }
      ...
      terminal.write(['Backup complete', chalk.dim(output_file), chalk.dim(size)])
    }
