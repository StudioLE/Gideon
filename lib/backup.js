...
module.exports = function() {
  ...
  async.waterfall([
    ...
    function(files, callback) { // INPUT: Select source directory
      ...
      if(arg[1] && _.contains(files, arg[1])) {
    ...
    function(db_found, callback) { // INPUT: Backup MySQL?
      ...
      if(db_found && _.contains([true, 'yes', 'y'], arg[2])) {
        callback(null, 'Yes', null)
      }
      else if( ! db_found || _.contains([false, 'no', 'n'], arg[2])) {
        callback(null, 'No', 1)
      }
      else {
    ...
    function(backup_database, index, callback) { // Backup directory & MySQL
      ...
      // Output file name
      var output = {}
      if(config.destination[config.target].unique_name) {
        output.name = source.name + '-' + util.machine_date() + '.' + config.format
      }
      else {
        output.name = source.name + '.' + config.format
      }
      ...
      if(config.target == 'local') {
        ...
        if(config.destination.s3.directory) {
          output.path = config.destination.s3.directory + '/' + output.name
        }
        else {
