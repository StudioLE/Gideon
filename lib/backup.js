...
module.exports = function() {
  ...
  async.waterfall([
      ...
      if(config.target == 'local') {
        ...
        */
        output.stream.on('part', function(details) {
          ...
          terminal.write(['Running backup', chalk.dim(output.name), chalk.dim(upload_size) + ' so far'])
        })
    ...
    if(err){
      ...
      terminal.write(['Backup complete', chalk.dim(file), chalk.dim(size), chalk.dim(time)])
    }
