...
module.exports = function() {
  ...
  async.waterfall([
    ...
    function(files, callback) { // INPUT: Select source directory
      ...
      sqwk.write('Select a directory to backup', files, callback)
    ...
    function(backup_database, index, callback) { // Backup directory & MySQL
      ...
      sqwk.write(['Running backup', chalk.dim(output.name)])
      ...
      if(config.target == 'local') {
        ...
        */
        output.stream.on('part', function(details) {
          ...
          sqwk.write(['Running backup', chalk.dim(output.name), chalk.dim(upload_size) + ' so far'])
        })
    ...
    if(err){
      ...
      sqwk.write(['Backup complete', chalk.dim(file), chalk.dim(size), chalk.dim(time)])
    }
